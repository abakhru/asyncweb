...
#
# @router.root('/home', status_code=200)
# def home():
#     # if not session.get('logged_in'):
#     return app.render_template(APP_HOME.joinpath("templates", "login.html"))
#     # else:
#     #     return JSONResponse(status_code=200, content={"message": "Welcome to Demo Server"})


@router.post('/login', responses=get_token_response)
async def login(user: UserAuthenticate, db: Session = Depends(get_db)) -> JSONResponse:
